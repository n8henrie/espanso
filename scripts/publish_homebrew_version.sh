...
INTEL_SHA=$(awk -F ' ' '{print $1}' target/homebrew/artifacts/Espanso-Mac-Intel.zip.sha256.txt)
M1_SHA=$(awk -F ' ' '{print $1}' target/homebrew/artifacts/Espanso-Mac-M1.zip.sha256.txt)
...
sed < ../../../scripts/resources/macos/formula_template.rb \
  ...
  -e "s/{{{M1_SHA}}}/${M1_SHA}/g" > ./Casks/espanso.rb
